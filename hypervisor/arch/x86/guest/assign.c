...
ptirq_build_physical_rte(struct acrn_vm *vm, struct ptirq_remapping_info *entry)
{
	...
	vioapic_get_rte(vm, virt_sid->ioapic_id.gsi, &virt_rte);
	rte = virt_rte;

	/* init polarity & pin state */
	if (rte.bits.intr_polarity == IOAPIC_RTE_INTPOL_ALO) {
		if (entry->polarity == 0U) {
			vioapic_set_irqline_nolock(vm, virt_sid->ioapic_id.gsi, GSI_SET_HIGH);
		}
		entry->polarity = 1U;
	} else {
		if (entry->polarity == 1U) {
			vioapic_set_irqline_nolock(vm, virt_sid->ioapic_id.gsi, GSI_SET_LOW);
		}
		entry->polarity = 0U;
	}
...
static struct ptirq_remapping_info *add_intx_remapping(struct acrn_vm *vm,
		uint32_t virt_gsi, uint32_t phys_gsi)
{
	struct ptirq_remapping_info *entry = NULL;
	DEFINE_INTX_SID(phys_sid, phys_gsi);
	DEFINE_INTX_SID(virt_sid, virt_gsi);
	...
	if (entry == NULL) {
		...
		if (is_sos_vm(entry->vm)) {
			...
			pr_err("INTX gsi%d already in vm%d with vgsi%d, not able to add into vm%d with vgsi%d",
					phys_gsi, entry->vm->vm_id, entry->virt_sid.ioapic_id.gsi, vm->vm_id, virt_gsi);
			entry = NULL;
		}
	} else {
		/* The mapping has already been added to the VM. No action
...
static void remove_intx_remapping(const struct acrn_vm *vm, uint32_t virt_gsi)
{
	uint32_t phys_irq;
	struct ptirq_remapping_info *entry;
	struct intr_source intr_src;
	...
	if (entry != NULL) {
		if (is_entry_active(entry)) {
			...
			dev_dbg(DBG_LEVEL_IRQ,
				"deactive intx entry:pgsi=%d, pirq=%d from vm%d vgsi=%d\n",
				entry->phys_sid.ioapic_id.gsi, phys_irq,
		...
		const struct ptirq_remapping_info *entry)
{
	...
	vioapic_get_rte(vm, (uint32_t)virt_sid->ioapic_id.gsi, &rte);
	...
	if (trigger_lvl) {
		if (entry->polarity != 0U) {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic_id.gsi, GSI_SET_LOW);
		} else {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic_id.gsi, GSI_SET_HIGH);
		}
	} else {
		if (entry->polarity != 0U) {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic_id.gsi, GSI_FALLING_PULSE);
		} else {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic_id.gsi, GSI_RAISING_PULSE);
		}
	}

	dev_dbg(DBG_LEVEL_PTIRQ, "dev-assign: irq=0x%x assert vr: 0x%x vRTE=0x%lx",
		entry->allocated_pirq, irq_to_vector(entry->allocated_pirq), rte.full);
}
...
void ptirq_intx_ack(struct acrn_vm *vm, uint32_t virt_gsi)
{
	uint32_t phys_irq;
	struct ptirq_remapping_info *entry;
	DEFINE_INTX_SID(virt_sid, virt_gsi);
	...
	if (entry != NULL) {
		 ...
		 * means we have this vioapic enabled
		 */
		if (entry->polarity != 0U) {
...
int32_t ptirq_intx_pin_remap(struct acrn_vm *vm, uint32_t virt_gsi)
{
	int32_t status = 0;
	struct ptirq_remapping_info *entry = NULL;
	DEFINE_INTX_SID(virt_sid, virt_gsi);
	...
	if (!is_vuart_intx(vm, virt_sid.ioapic_id.gsi)) {
...
/* @pre vm != NULL
 * except sos_vm, Device Model should call this function to pre-hold ptdev intx entries:
 * - the entry is identified by phys_pin:
 *   one entry vs. one phys_pin
 * - currently, one phys_pin can only be held by one pin source (vIOAPIC)
 */
int32_t ptirq_add_intx_remapping(struct acrn_vm *vm, uint32_t virt_gsi, uint32_t phys_gsi)
{
	struct ptirq_remapping_info *entry;
...
{
	spinlock_obtain(&ptdev_lock);
	remove_intx_remapping(vm, virt_gsi);
	spinlock_release(&ptdev_lock);
}
